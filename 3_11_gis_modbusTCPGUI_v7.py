...
class MainWindow(QMainWindow):
    ...
    def create_content_area(self):
        ...
        # 设置分割比例
        content_splitter.setSizes([300, 750])
    ...
    def create_control_panel(self):
        ...
        # 创建水平布局用于放置API检查和历史按钮
        api_history_layout = QHBoxLayout()
        api_history_layout.setSpacing(8)  # 设置水平布局的间距
        api_history_layout.setContentsMargins(0, 5, 0, 5)  # 设置边距
        
        # 添加API服务器连接按钮
        self.check_api_button = QPushButton("检查API服务")
        self.check_api_button.clicked.connect(self.check_api_connection)
        self.check_api_button.setMinimumWidth(130)  # 设置最小宽度
        self.check_api_button.setFixedHeight(32)  # 设置固定高度
        self.check_api_button.setStyleSheet("""
            QPushButton {
                font-size: 12px;
                padding: 4px;
                font-weight: bold;
        ...
        """)
        api_history_layout.addWidget(self.check_api_button)
        
        # 添加查看历史记录按钮
        self.history_button = QPushButton("查看历史记录")
        self.history_button.clicked.connect(self.show_history_records)
        self.history_button.setMinimumWidth(130)  # 设置最小宽度
        self.history_button.setFixedHeight(32)  # 设置固定高度
        ...
        """)
        api_history_layout.addWidget(self.history_button)
        
        # 将水平布局添加到主布局
        recognition_layout.addLayout(api_history_layout)
